...
class _DefaultExCallback(object):
    ...
    def __call__(self, ex: sacred.Experiment, mode, k, v):
        if mode == 'train':
            ...
            ex.log_scalar("training.{}".format(k), self.train_emas[k])

        elif mode == 'val':
            #ex.log_scalar(f'val.{k}', np.mean(np.array(v)))
            #ex.log_scalar(f'train.{k}', np.mean(np.array(self.train_vals[k])))
            ex.log_scalar("val.{}".format(k), np.mean(np.array(v)))
            ex.log_scalar("train.{}".format(k), np.mean(np.array(self.train_vals[k])))
            self.train_vals[k] = []
