...
def load_mean_smpl_parameters(smpl_mean_theta_path):
    mean = np.zeros(82, dtype = np.float)#args.total_theta_count

    mean_values = h5py.File(smpl_mean_theta_path)
    mean_pose = mean_values['pose']
    mean_pose[:3] = 0
    mean_shape = mean_values['shape']

    # This initializes the global pose to be up-right when projected
    # it is not necessary for our problem, jjcao, then facing z+ & heading y+
    #mean_pose[0]=np.pi
...
def center_pts_tensor(inpts, bBottom = True):
    dim = len(inpts.shape)
    if dim == 3:
        startDim = 1
    else: # dim == 2
    ...
    bbox_max, _ = torch.max(inpts, startDim)
    bbox_min, _ = torch.min(inpts, startDim)
    translation = (bbox_max + bbox_min) / 2.0 #(B, 3)
    if bBottom:
        if startDim == 0:
            translation[1] = bbox_min[1] # make the lowest y = 0 after centerization
        else:
            translation[:,1] = bbox_min[:,1] # make the lowest y = 0 after centerization
    return translation

def center_pts_np_array(inpts, bBottom = True):
    ...
    if bBottom:
        translation[1] = bbox[1][1] # make the lowest y = 0 after centerization, bbox[1,1]
    pts = inpts - translation
...
# poor performance for SPENetwork
def weights_init(m):
...
def batch_rodrigues(theta):
    '''
        todo replaced by Li algebra, Junqi Lin @ jjcao

        original approach: rodrigues: from axis-angle representation to matrix representation
        the approach now: quaternion: from axis-angle representation to quaternion representation, & matrix representation        
...
def batch_global_rigid_transformation(Rs, Js, parent, rotate_base = False):
    ...
    if rotate_base:
        ...
        np_flip = np.array([[1, 0, 0], [0, 1, 0], [0, 0, -1]], dtype = np.float) # flip
        np_flip = np.reshape(np.tile(np_flip, [N, 1]), [N, 3, 3])
        flip = Variable(torch.from_numpy(np_flip).float())

        np_rot_x = np.array([[1, 0, 0], [0, 0, -1], [0, 1, 0]], dtype = np.float) # rotate        
        ...
        tmp = torch.matmul(rot_x, flip)
        #root_rotation = torch.matmul(Rs[:, 0, :, :],  rot_x)
        root_rotation = torch.matmul(Rs[:, 0, :, :],  tmp)
    else:
        root_rotation = Rs[:, 0, :, :]
